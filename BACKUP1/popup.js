// popup.js - Version 0.8.6 (Corrected ADD_TRIGGER message type)

document.addEventListener('DOMContentLoaded', () => {
    console.log("Popup: DOM fully loaded and parsed (v0.8.6).");
    ...
    async function loadTranslations() {
        ...
        try {
            ...
            if (uiLocale !== 'en') {
                try {
                    const resp = await fetch(BROWSER_API.runtime.getURL(`_locales/${uiLocale}/messages.json`));
                    if (resp.ok) { messagesData = await resp.json(); loadedLocale = uiLocale; }
                    else { console.warn(`Popup: Failed locale fetch '${uiLocale}' (Status: ${resp.status}).`); }
            ...
            if (!messagesData) {
                ...
                if (!resp.ok) throw new Error(`Fallback fetch fail: ${resp.status}`);
                messagesData = await resp.json(); loadedLocale = 'en';
            }
            state.translations = messagesData;
            state.translationsReady = true;
            console.log(`Popup: Translations ready (using '${loadedLocale}'). Applying...`);
            applyStaticTranslations();
        } catch (e) {
            console.error("Popup: CRITICAL failure loading translations:", e);
            applyStaticTranslations(true);
            showStatus("popupErrorLoadingText", 'error', 0);
        }
    }
    ...
    function applyStaticTranslations(useFallback = false) {
        ...
        try {
            ...
            console.error("Popup: Error applying translations to DOM:", e);
        }
    ...
    function populateCategoryDropdown() {
        if (!elements.categorySelect) {
            console.error("Popup: Category select element not found."); return;
        }
        const currentVal = elements.categorySelect.value;
        elements.categorySelect.innerHTML = '';
        const defaultOption = document.createElement('option');
        defaultOption.value = "";
        defaultOption.textContent = getMsg("popupCategoryDefaultOption", "-- Select a Category --");
        ...
        const sortedTriggerKeys = Object.keys(availableTriggers).sort((a, b) => {
            const nameA = getMsg(availableTriggers[a], a).toLowerCase();
            const nameB = getMsg(availableTriggers[b], b).toLowerCase();
            return nameA.localeCompare(nameB);
        });
        for (const triggerKey of sortedTriggerKeys) {
            const messageKey = availableTriggers[triggerKey];
            const fallbackName = triggerKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
            const categoryName = getMsg(messageKey, fallbackName);
            const option = document.createElement('option');
            option.value = triggerKey;
            option.textContent = categoryName;
            if (triggerKey === currentVal) option.selected = true;
            elements.categorySelect.appendChild(option);
        }
    }

    async function fetchVideoState(retryCount = 0) {
        if (!state.currentTabId || !state.isNetflixWatchTab) {
            console.log("Popup: Not fetching state - Conditions not met.");
            showStatus("popupErrorNotNetflix", 'warning', 5000);
            return null;
        }
        ...
        try {
            ...
            if (!response) throw new Error("No response received from content script.");
            ...
            if (response.videoId !== undefined && typeof response.currentTime === 'number' && response.currentTime >= 0) {
                if (state.currentVideoId !== response.videoId && response.videoId !== null) {
                    console.log(`Popup: Updated videoId from ${state.currentVideoId} to ${response.videoId}`);
                    state.currentVideoId = response.videoId;
                    state.markedStartTime = null; state.markedEndTime = null;
                    if (elements.startTimeDisplay) elements.startTimeDisplay.textContent = formatTime(null);
                    if (elements.endTimeDisplay) elements.endTimeDisplay.textContent = formatTime(null);
                } else if (response.videoId === null && state.currentVideoId !== null) {
                     console.log("Popup: Received null videoId, clearing stored ID.");
                     state.currentVideoId = null;
                     setControlsDisabledState(true);
                     showStatus("popupErrorNoVideoId", 'warning', 5000);
                }
                state.isVideoPlaying = response.isPlaying ?? false;
                updatePlaybackButtons();
                updateSubmitButtonState();
                return response.currentTime;
            } else {
                 console.warn(`Popup: Received invalid state response structure:`, response);
                 state.isVideoPlaying = response?.isPlaying ?? false;
                 updatePlaybackButtons(); updateSubmitButtonState();
                 throw new Error(`Invalid response structure received from content script.`);
            }
        } catch (error) {
            console.error(`Popup: Error during fetchVideoState (Attempt ${retryCount + 1}):`, error.message);
            ...
            if (isConnError && retryCount < 2) {
                const delay = 300 * Math.pow(2, retryCount);
                console.log(`Popup: Connection error, retrying fetchVideoState in ${delay}ms...`);
                await new Promise(resolve => setTimeout(resolve, delay));
                return fetchVideoState(retryCount + 1);
            }
            state.currentVideoId = null; state.isVideoPlaying = false;
            const errorKey = isConnError ? "popupErrorConnection" : "popupErrorGetTime";
            showStatus(errorKey, 'error', 5000);
            setControlsDisabledState(true); updateSubmitButtonState();
            return null;
        }
    }
    ...
    async function sendPlayPauseCommand(command) {
         if (!state.currentTabId || !state.isNetflixWatchTab) {
             showStatus("popupErrorNotNetflix", 'error'); return;
         }
         const targetPlayingState = (command === "PLAY_VIDEO");
         state.isVideoPlaying = targetPlayingState;
         updatePlaybackButtons(); showStatus('');
         console.log(`Popup: Sending ${command} to tab ${state.currentTabId}`);
         try {
             await BROWSER_API.tabs.sendMessage(state.currentTabId, { type: command });
             console.log(`Popup: ${command} sent successfully.`);
         }
         catch (error) {
             console.error(`Popup: Error sending ${command}:`, error);
             const isConnErr = error.message?.includes("connection") || error.message?.includes("Receiving end does not exist") || error.message?.includes("Port closed");
             showStatus(isConnErr ? "popupErrorConnection" : "popupErrorPlayPause", 'error');
             state.isVideoPlaying = !targetPlayingState; updatePlaybackButtons();
         }
    }

    async function handleMarkStart() {
        if (!state.isNetflixWatchTab || elements.markStartBtn?.disabled) return;
        console.log("Popup: Mark Start clicked."); showStatus('');
        const time = await fetchVideoState();
        if (time !== null) {
            state.markedStartTime = time;
            if (elements.startTimeDisplay) elements.startTimeDisplay.textContent = formatTime(state.markedStartTime);
            console.log(`Popup: Start time marked at ${formatTime(state.markedStartTime)} (${state.markedStartTime}s)`);
            if (state.markedEndTime !== null && state.markedEndTime <= state.markedStartTime) {
                 console.log("Popup: Clearing end time because it's <= new start time.");
                state.markedEndTime = null;
                if (elements.endTimeDisplay) elements.endTimeDisplay.textContent = formatTime(null);
                showStatus("popupWarnEndTimeCleared", 'warning', 2500);
            }
        } else { console.log("Popup: Could not mark start time (fetch failed)."); }
        updateSubmitButtonState();
    }

    async function handleMarkEnd() {
        if (!state.isNetflixWatchTab || elements.markEndBtn?.disabled) return;
        console.log("Popup: Mark End clicked."); showStatus('');
        const time = await fetchVideoState();
        if (time !== null) {
            if (state.markedStartTime === null || time <= state.markedStartTime) {
                console.log("Popup: Invalid end time (before or same as start time).");
                showStatus("popupErrorEndTime", 'error', 4000);
                state.markedEndTime = null;
                if (elements.endTimeDisplay) elements.endTimeDisplay.textContent = formatTime(null);
            } else {
                state.markedEndTime = time;
                if (elements.endTimeDisplay) elements.endTimeDisplay.textContent = formatTime(state.markedEndTime);
                 console.log(`Popup: End time marked at ${formatTime(state.markedEndTime)} (${state.markedEndTime}s)`);
            }
        } else { console.log("Popup: Could not mark end time (fetch failed)."); }
        updateSubmitButtonState();
    }

    async function handleSubmitTrigger() {
        if (elements.submitBtn?.disabled || state.isSubmitting) {
            console.log("Popup: Submit prevented (disabled or already submitting)."); return;
        }
        showStatus('');
        ...
        if (!state.currentVideoId) {
             console.log("Popup: Submit prevented - trying to fetch video ID again.");
             await fetchVideoState();
             if (!state.currentVideoId) { showStatus("popupErrorNoVideoId", 'error', 4000); return; }
             console.log("Popup: Recovered video ID. Proceeding with submit.");
        }
        const selectedCategoryKey = elements.categorySelect?.value;
        const timesValid = state.markedStartTime !== null && state.markedEndTime !== null && state.markedEndTime > state.markedStartTime;
        ...
        state.isSubmitting = true; updateSubmitButtonState();
        showStatus("popupStatusSubmitting", 'submitting', 0);

        let responseReceived = false;
        const submitTimeoutMs = 20000;
        const submitTimeoutId = setTimeout(() => {
            if (!responseReceived && BROWSER_API.runtime?.id) {
                console.warn("Popup: Submit timed out."); state.isSubmitting = false;
                showStatus("popupErrorTimeout", 'error', 0); updateSubmitButtonState();
            }
        }, submitTimeoutMs);
        ...
        try {
            ...
            responseReceived = true; clearTimeout(submitTimeoutId);
            console.log("Popup: Received response from background for FIRESTORE_ADD_TRIGGER:", response);

            if (response?.status === "success") {
                console.log("Popup: Submission successful!");
                showStatus("popupStatusSuccess", 'success', 3500);
                state.markedStartTime = null; state.markedEndTime = null;
                if (elements.startTimeDisplay) elements.startTimeDisplay.textContent = formatTime(null);
                if (elements.endTimeDisplay) elements.endTimeDisplay.textContent = formatTime(null);
                if (elements.categorySelect) elements.categorySelect.value = "";
            } else {
                const errorKey = response?.code === 'permission-denied' ? "popupErrorPermissionDenied" : response?.message?.includes("duplicate") ? "popupErrorDuplicate" : "popupErrorSubmit";
                const errorMessage = response?.message || "Unknown error from background";
                console.error(`Popup: Submission failed. Status: ${response?.status}, Message: ${errorMessage}`);
                showStatus(errorKey, 'error', 5000);
            }
        } catch (error) {
            responseReceived = true; clearTimeout(submitTimeoutId);
            console.error("Popup: Error sending FIRESTORE_ADD_TRIGGER message or processing response:", error);
            const isConnError = error.message?.includes("Receiving end does not exist") || error.message?.includes("Could not establish connection") || error.message?.includes("Port closed");
            showStatus(isConnError ? "popupErrorConnection" : "popupErrorSubmit", 'error', 5000);
        } finally {
            if (BROWSER_API.runtime?.id) {
                state.isSubmitting = false; updateSubmitButtonState();
            }
        }
    }
    ...
    async function initializePopup() {
        ...
        if (elements.actionButtonsContainer) elements.actionButtonsContainer.style.display = 'flex';
        if (elements.settingsBtn) { elements.settingsBtn.style.display = 'inline-block'; elements.settingsBtn.disabled = false; }
        if (elements.supportBtn) { elements.supportBtn.style.display = 'inline-block'; elements.supportBtn.disabled = false; }
        ...
        try {
            ...
            console.log(`Popup: Active Tab ID: ${state.currentTabId}, URL: ${activeTab.url}`);
            state.isNetflixWatchTab = !!activeTab.url?.match(/^https?:\/\/www\.netflix\.com\/watch\/\d+/);
            if (!state.isNetflixWatchTab) {
                console.log("Popup: Not on a Netflix watch page.");
                showStatus("popupErrorNotNetflix", 'warning', 0); setControlsDisabledState(true);
            } else {
                console.log("Popup: Netflix watch page detected. Fetching initial video state...");
                setControlsDisabledState(false);
                await fetchVideoState();
                console.log("Popup: Initial video state fetched.");
            }
        } catch (e) {
            console.error("Popup initialization error:", e);
            const errorKey = e.message === "No active tab found." ? "popupErrorNoTab" : "popupErrorGeneric";
            showStatus(errorKey, 'error', 0); setControlsDisabledState(true);
        } finally {
             console.log("Popup: Attaching listeners...");
            attachListeners(); updateSubmitButtonState();
            console.log("Popup: Initialization complete.");
        }
    }

    function attachListeners() {
        if (elements.markStartBtn) elements.markStartBtn.addEventListener('click', handleMarkStart);
        if (elements.markEndBtn) elements.markEndBtn.addEventListener('click', handleMarkEnd);
        if (elements.categorySelect) elements.categorySelect.addEventListener('change', updateSubmitButtonState);
        if (elements.submitBtn) elements.submitBtn.addEventListener('click', handleSubmitTrigger);
        if (elements.playBtn) elements.playBtn.addEventListener('click', () => sendPlayPauseCommand("PLAY_VIDEO"));
        if (elements.pauseBtn) elements.pauseBtn.addEventListener('click', () => sendPlayPauseCommand("PAUSE_VIDEO"));
        if (elements.settingsBtn) {
            console.log("Popup: Attaching listener to Settings button.");
            elements.settingsBtn.addEventListener('click', () => {
               console.log("Popup: Settings button clicked.");
               BROWSER_API.runtime.sendMessage({ type: "OPEN_OPTIONS" }).catch(e => console.error("Popup: Error sending OPEN_OPTIONS:", e));
           });
        } else { console.error("Popup: Settings button element NOT found."); }
        if (elements.supportBtn) {
            console.log("Popup: Attaching listener to Support button.");
            elements.supportBtn.addEventListener('click', () => {
               console.log("Popup: Support button clicked.");
               if (!SUPPORT_LINK || SUPPORT_LINK.includes("YOUR_USERNAME_HERE")) {
                   console.error("Popup: Support link invalid!");
                   showStatus("popupErrorSupportLink", "error", 4000); return;
               }
                console.log(`Popup: Opening support link: ${SUPPORT_LINK}`);
               BROWSER_API.runtime.sendMessage({ type: "OPEN_TAB", url: SUPPORT_LINK }).catch(e => console.error("Popup: Error sending OPEN_TAB:", e));
           });
        } else { console.error("Popup: Support button element NOT found."); }
